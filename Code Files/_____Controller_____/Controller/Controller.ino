...
//  flash program
  else if(digitalRead(CAM_SW_PIN)==LOW)
      {
        delay(200);
        cam_btn_state = !cam_btn_state;
        if(cam_btn_state == false)
        { 
          cam_con="FLASHON";
          print_on_OLED("FLASHON");
        }
        else
        {
          cam_con="FLASHOFF";
          print_on_OLED("FLSHOFF");
        }
      }
  else
  {
...
//END OF JOYSTICK PROGRAM
  ...
  int sensor_v6 = analogRead(pot_Pin6);
  ...
  mot_val_6 = map(sensor_v6,0,4095,70,520);

  
  motor_val= motor_val + String(mot_val_1) + "-" + String(mot_val_2)+ "-" + String(mot_val_3)+ "-" + String(mot_val_4)+ "-" + String(mot_val_5) + "-" + String(mot_val_6);
