...
int values[6] = {0};                  // values to be given to arm servo motors
int values_o[6] = {0};
...
int front_ob=0,right_ob=0,left_ob=0;  // front_ob -> distance_front, right_ob -> distance_right 
                        // left_ob -> distance_left
...
//------variables
  std::string rxval_rov;
  std::string rxval_cam;
  std::string rxval_arm;
//-------
...
// The remote service we wish to connect to.
  ...
  static BLEUUID CHARACTERISTIC_UUI_ROV("beb5483e-36e1-4688-b7f5-ea07361b26a8");
...
bool connectToServer(BLEAddress pAddress) 
{
 ...
 sv_wri_rov = pRemoteService->getCharacteristic(CHARACTERISTIC_UUI_ROV);
...
void rover_fn(std::string rxval_rov)
{
    ...
    else
    {
      ...
      if(rxval_rov == "UP")
      {
          front_ob = cal_dist(trigPin_f, echoPin_f);
          if(front_ob < 10)
          {
            stopMotors();
      ...
      {
          left_ob = cal_dist(trigPin_l,  echoPin_l);
          if(left_ob<10)
          {
            stopMotors();
            digitalWrite(BUZZER_PIN, LOW);//on
      ...
      {
          right_ob = cal_dist(trigPin_r, echoPin_r);
          if(right_ob<10)
          {
            stopMotors();
            digitalWrite(BUZZER_PIN, LOW);//on
      ...
      sv_wri_rov->writeValue("");
}

void cam_fn(std::string rxval_cam ){
  if(rxval_cam=="NORMAL")
  {
    return;
  }
    ...
    if(rxval_cam == "FLASHON")
    {
      digitalWrite(flash_pin, HIGH);
    }

    else if(rxval_cam == "FLASHOFF")
    {
...
void arm_fn(std::string rxval_arm){
      ...
      for (int i = 0; i < 6; i++) 
      {
        ...
        Serial.print(i + 1);
        Serial.print(": ");
        Serial.println(values_o[i]);
      }

      for(int i=0; i<6; i++)
      {
        pwm.setPWM(i+4,0,values_o[i]);
      }
...
void loop() 
{
  ...
  rxval_rov = sv_wri_rov->readValue();
  rxval_cam = sv_wri_cam->readValue();
  rxval_arm = sv_wri_arm->readValue();
